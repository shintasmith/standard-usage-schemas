...
Attribute NameDescriptionTypeOptionality``tenants``

Specifies the Ids of the tenant associated with this token, if any.
...
Specifies the date to compare against a token's creation date. Tokens that were created before this date (and match the other criteria in the TRR) should be considered revoked

utcDateTime

Required

Attribute group ``tokenAuthenticatedBy`` : Specifies a set of authentication method(s) against which to compare to a token's authenticatedBy attribute. If no tokenAuthenticatedBy element is included in the TRR, then this attribute will not be used to limit which tokens are considered revoked.

The ``tokenAuthenticatedBy`` element may occur zero to 10 times and takes the following attributes:
...
``PASSWORD``, ``APIKEY``, ``PASSCODE``, ``RSAKEY``, ``FEDERATION``
...
Attribute NameDescriptionTypeOptionality``displayName``
...
``PASSWORD``, ``ROLES``, ``GROUPS``, ``FIRSTNAME``
